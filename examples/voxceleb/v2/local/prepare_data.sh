...
rawdata_dir=/hpctmp/e0643891/sitw/
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
  ...
  rawdata_dir=/data_a11/mayi/dataset/VoxCeleb_latest/VoxCeleb1
  mkdir -p data/vox1
  # musan
  #find /hpctmp/e0643891/dataset_vox1/musan -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' >data/musan/wav.scp
  # rirs
  #find /hpctmp/e0643891/dataset_vox1/RIRS_NOISES/simulated_rirs -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' >data/rirs/wav.scp
  # vox1
  #find ${rawdata_dir}/wav -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' | sort >${data}/vox1/wav.scp
  
  #find ${rawdata_dir}/wav -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' | sort >data/vox1_O/wav.scp
  ...
  #fi
  # vox2
  find /home2/mayi_data/dataset/voxceleb/voxceleb2/voxceleb2_wav -name "*.wav" | awk -F"/" '{print $(NF-2)"/"$(NF-1)"/"$NF,$0}' | sort >${data}/vox2/wav.scp
  awk '{print $1}' ${data}/vox2/wav.scp | awk -F "/" '{print $0,$1}' >${data}/vox2/utt2spk
  ./tools/utt2spk_to_spk2utt.pl ${data}/vox2/utt2spk >${data}/vox2/spk2utt
...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
  ...
  rawdata_dir=/data_a11/mayi/dataset/librispeech/37_Librispeech/noisy/clean_all
