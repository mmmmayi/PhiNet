...
set -e
data_name_array=("librispeech")
data_list_path_array=( "${data}/${data_name_array}/${data_type}.list")
echo ${data_list_path_array}
data_scp_path_array=( "data/${data_name_array}/wav.scp") # to count the number of wavs
...
for i in $(seq 0 $(($count - 1))); do
  ...
  bash tools/extract_embedding.sh --dur ${dur} \
    --exp_dir ${exp_dir} \
