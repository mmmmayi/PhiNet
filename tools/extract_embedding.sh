...
nj=4
gpus="[0,1]"
pho_path='/scratch/e0643891/dataset_vox1/voxceleb1/wav'
dele=
...
for suffix in $(seq 0 $(($nj - 1))); do
  ...
  pho_ark=${embed_dir}/phovector_${suffix}.ark
...
cat ${embed_dir}/phovector_00*.scp >${embed_dir}/phovector_${dur}s.scp
#cat ${embed_dir}/xvector_00*.scp >${embed_dir}/xvector_${dur}s.scp
embed_num=$(wc -l ${embed_dir}/phovector_${dur}s.scp | awk '{print $1}')
