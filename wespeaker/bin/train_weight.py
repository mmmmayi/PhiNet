...
def train(config='conf/config.yaml', **kwargs):
    ...
    optimizer = getattr(torch.optim,
                        configs['optimizer'])(ddp_model.module.weight.parameters(),
    ...
    weight = [configs['weight_pho'],configs['weight_verf']]
    
    if rank == 0:
        logger.info("<== weight ==>")
        logger.info("weight loss pho is: " + str(configs['weight_pho']))
    ...
    if rank == 0:
        ...
        header = ['Epoch', 'Batch', 'Lr',  'Loss_pho','Loss_verf']
    ...
    if rank == 0:
        os.symlink('model_{}.pt'.format(configs['num_epochs']),
                   os.path.join(model_dir, 'final_model_weight.pt'))
