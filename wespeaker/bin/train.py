...
def train(config='conf/config.yaml', **kwargs):
    ...
    #torch.save(spk2id_dict,'/data_a11/mayi/project/SIP/IRM/exp/resnet_babble_speaker.pt')
    #quit()
    if rank == 0:
    ...
    batch_size = configs['dataset_args']['num_spk']*configs['dataset_args']['num_utts_per_spk']
    train_dataloader = DataLoader(train_dataset,batch_size=batch_size, **configs['dataloader_args'])
    ...
    weight = [configs['weight_cstr'],configs['weight_diff'],configs['weight_selfcstr'],configs['weight_verf']]
    if configs['weight_args']['loss']=='training':
        mean = torch.mean(torch.from_numpy(np.load('exp/train_ratio.npy')).float()).unsqueeze(0)
        phoneme_weight=torch.cat((torch.from_numpy(np.load('exp/train_ratio.npy')).float(),mean)).cuda()
        phoneme_weight=phoneme_weight.reshape(40).unsqueeze(0)
        max = torch.amax(phoneme_weight,dim=-1).unsqueeze(-1)
        min = torch.amin(phoneme_weight,dim=-1).unsqueeze(-1)
        phoneme_weight = (phoneme_weight-min)/(max-min)
    elif configs['weight_args']['loss']=='librispeech':
        mean = torch.mean(torch.from_numpy(np.load('exp/val.npy')).float()).unsqueeze(0)
        phoneme_weight = torch.cat((torch.from_numpy(np.load('exp/val.npy')).float(),mean)).cuda()
        phoneme_weight=phoneme_weight.reshape(40).unsqueeze(0)
        max = torch.amax(phoneme_weight,dim=-1).unsqueeze(-1)
        min = torch.amin(phoneme_weight,dim=-1).unsqueeze(-1)
        phoneme_weight = (phoneme_weight-min)/(max-min)
    ...
    if rank == 0:
        logger.info("<== weight ==>")
        logger.info("weight loss cstr is: " + str(configs['weight_cstr']))
        logger.info("weight loss diff is: " + str(configs['weight_diff']))
        logger.info("weight loss self-cstr is: " + str(configs['weight_selfcstr']))
        logger.info("weight loss verf is: " + str(configs['weight_verf']))
    ...
    configs['scheduler_args']['scale_ratio'] = 1.0 * world_size * batch_size / 64
    ...
    if rank == 0:
        ...
        header = ['Epoch', 'Batch', 'Lr',  'Loss_cstr','Loss_diff','Loss_selfcstr','Loss_verf']
