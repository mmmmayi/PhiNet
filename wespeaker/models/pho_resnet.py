...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv1 = self.fill_fix(nn.Conv1d(in_planes,
                               planes,
                               kernel_size=3,
                               stride=stride,
                               padding=1))
        self.conv2 = self.fill_fix(nn.Conv1d(planes,
                               planes,
                               kernel_size=3,
                               stride=1,
                               padding=1))
        ...
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                self.fill_fix(nn.Conv1d(in_planes,
                          self.expansion * planes,
                          kernel_size=1,
                          stride=stride)))

    def fill_fix(self,block):
        block.weight.data.fill_(1)
        block.bias.data.fill_(0)

        block.weight.requires_grad = False
        block.bias.requires_grad = False
        return block
...
class ResNet(nn.Module):
    def __init__(self,
        ...
        self.conv1 = self.fill_fix(nn.Conv1d(1,
                               m_channels,
                               kernel_size=3,
                               stride=1,
                               padding=1))
        ...
        self.layer4 = self._make_layer(block,
                                       ...
                                       num_blocks[3],
                                       stride=2)
        self.win_eye = torch.eye(400).unsqueeze(1)
    ...
    def forward(self, x):
        #print(x.shape)#[2, 32240]
        #x = conv1d(x.float(), self.win_eye, stride=160).squeeze()
